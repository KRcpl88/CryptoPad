                     ...
                     _In_ int       nCmdShow)
{
    ...
    if (ppArgs != nullptr && cArgs >= 2)
    {
        ...
        if (cArgs < 4)
        {
            ::MessageBoxW(nullptr, L"Usage: CryptoPad.exe <E|D> <filename> <password>", L"Argument Error", MB_OK | MB_ICONERROR);
            nResult = 1;
            goto Error;
        }
...
void EncryptFile(__in_z LPCWSTR pszFilename, __in_z const LPCWSTR pszPassword)
{
    ...
    unsigned char* pKey = new unsigned char[CP_CIPHER::s_GetKeyWidth()];
    ...
    ::FbcEncryptFile(pszFilename, szEncryptedFile, pKey, CP_CIPHER::s_GetKeyWidth());
...
void DecryptFile(__in_z LPCWSTR pszFilename, __in_z const LPCWSTR pszPassword)
{
    ...
    *pszExt = 0;    // truncate extension
    ...
    ::FbcDecryptFile(pszFilename, szDecryptedFile, pKey, CP_CIPHER::s_GetKeyWidth());
...
{
    unsigned char* pKey = new unsigned char[CP_CIPHER::s_GetKeyWidth()];
    CP_CIPHER* pFbcEncrypt = new CP_CIPHER();
    int cbText = ::lstrlenW(pszText) * sizeof(*pszText);
    int cBlocks = (cbText / k_cAesBlockSizeBytes) + 1;
    BYTE* pNonce = new BYTE[k_cNonceSizeBytes];
    unsigned char* pBuffer = new unsigned char[k_cAesBlockSizeBytes * cBlocks];
    ...
    ::ZeroMemory(pBuffer, k_cAesBlockSizeBytes * cBlocks);
    ...
    if (!::ApplyNonce(pNonce, pKey, CP_CIPHER::s_GetKeyWidth(), pFbcEncrypt))
    {
        goto Error;
    }

    pFbcEncrypt->Encrypt(pBuffer, k_cAesBlockSizeBytes * cBlocks);
    ...
    fOK = ::WriteFile(hFileOut, pNonce, static_cast<DWORD>(k_cNonceSizeBytes), &dwBytesWritten, NULL);
    if ((!fOK) || (dwBytesWritten != (k_cNonceSizeBytes)))
    {
        goto Error;
    }
    ...
    fOK = WriteFile(hFileOut, pBuffer, k_cAesBlockSizeBytes * cBlocks, &dwBytesWritten, NULL);
    if ((!fOK) || (dwBytesWritten != k_cAesBlockSizeBytes * cBlocks))
    {
        goto Error;
    }
...
{
    unsigned char* pKey = new unsigned char[CP_CIPHER::s_GetKeyWidth()];
    CP_CIPHER* pFbcDecrypt = new CP_CIPHER();
    int cbText = 0;
    int cBlocks = 0;
    BYTE* pNonce = new BYTE[k_cNonceSizeBytes];
    ...
    ::ZeroMemory(pNonce, k_cNonceSizeBytes);
    fOK = ::ReadFile(hFileIn, pNonce, static_cast<DWORD>(k_cNonceSizeBytes), &dwBytesRead, NULL);
    if ((!fOK) || (dwBytesRead != k_cNonceSizeBytes))
    {
        goto Error;
    }
    ...
    cBlocks = (cbText / k_cAesBlockSizeBytes) + 1;
    pBuffer = new unsigned char[k_cAesBlockSizeBytes * cBlocks +1];
    ::ZeroMemory(pBuffer, k_cAesBlockSizeBytes * cBlocks);

    fOK = ::ReadFile(hFileIn, pBuffer, k_cAesBlockSizeBytes * cBlocks, &dwBytesRead, NULL);
    if ((!fOK) || (dwBytesRead != k_cAesBlockSizeBytes * cBlocks))
    {
        goto Error;
    }
    ...
    if (!::ApplyNonce(pNonce, pKey, CP_CIPHER::s_GetKeyWidth(), pFbcDecrypt))
    {
        goto Error;
    }

    pFbcDecrypt->Decrypt(pBuffer, k_cAesBlockSizeBytes * cBlocks);
